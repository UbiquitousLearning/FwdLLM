...
class FedAVGAggregator(object):
    ...
    def aggregate(self,current_round):
        ...
        model_list = []
        training_num = 0
        # logging.info(f"self.warmup_rounds {self.warmup_rounds}")

        if current_round < self.warmup_rounds:
            ratio = float(current_round) / float(max(1, self.warmup_rounds))
            # logging.info(f"float(current_round) / float(max(1, self.warmup_rounds)) {float(current_round) / float(max(1, self.warmup_rounds))}")
        else:
            ratio = max(
        ...
        for id,k in enumerate(old_param.keys()):
        ...
        self.set_global_model_params(old_param)
    ...
    def client_sampling(self, round_idx, client_num_in_total, client_num_per_round):
        ...
        index_list = [[] for _ in range(1)]

        for i in range(len(client_indexes)):
            index_list[i%1].append(client_indexes[i])
