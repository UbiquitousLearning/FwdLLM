...
class FedAVGAggregator(object):
    ...
    def add_local_trained_result(self, index, model_params, sample_num):
        ...
        if self.model_dict == {}:
            for key in model_params.keys():
                self.model_dict[key] = model_params[key]
        else:
            for key in model_params.keys():
                self.model_dict[key] += model_params[key]
        # self.sample_num_dict[self.cur] = sample_num
        self.cur += 1
    ...
    def check_whether_all_receive(self):
        ...
        # return True
        #     self.flag_client_model_uploaded_dict[idx] = False
        if self.cur == self.args.client_num_per_round:
            self.cur = 0
            for key in self.model_dict.keys():
                self.model_dict[key] = self.model_dict[key] / self.args.client_num_per_round
            return True
        return False
    ...
    def aggregate(self):
        ...
        # for idx in range(self.worker_num):
            ...
            #     self.model_dict[idx] = dequantize_params(self.model_dict[idx])
        # for idx in range(self.args.client_num_per_round):
        #     model_list.append((self.sample_num_dict[idx], self.model_dict[idx]))
        #     training_num += self.sample_num_dict[idx]
            # for i in range(len(self.model_dict[idx])):
            #     model_list.append((self.sample_num_dict[idx][i], self.model_dict[idx][i]))
            #     training_num += self.sample_num_dict[idx][i]
        # logging.info(len(model_list))
        # logging.info(training_num)
        ...
        self.set_global_model_params(copy.deepcopy(self.model_dict))
        self.model_dict = {}
