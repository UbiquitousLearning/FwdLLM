...
class ForwardTextClassificationTrainer:
    def __init__(self, args, device, model, train_dl=None, test_dl=None):
        ...
        self.adhoc_v_num_list = [1]*20 + [3] * 10 + [5]*100
        ...
        with torch.no_grad():
            for epoch in range(0, self.args.epochs):
                for batch_idx, batch in enumerate(self.train_dl):
                    # for _ in range(self.trainable_module_num):
                    for _ in range(self.adhoc_v_num_list[self.cur_v_num_index]):
                        ...
                        v_params = [torch.randn_like(p) if p.requires_grad == True else torch.zeros_like(p) for n,p in self.model.named_parameters()]
