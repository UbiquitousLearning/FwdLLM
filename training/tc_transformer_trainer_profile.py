            ...
            for epoch in range(0, self.args.epochs):
                for batch_idx, batch in enumerate(self.train_dl):
                    if batch_idx == 300:
                        ...
                        print(f"avg forward: {sum(forward_time)/len(forward_time)}")
                        print(f"avg calculate_jvp: {sum(calculate_jvp)/len(calculate_jvp)}")
                        print(f"avg calculate_vjp: {sum(calculate_vjp)/len(calculate_vjp)}")
                    ...
                    t1 = time.time()
                    # Backward AD
                    loss, vjp = fc.vjp(f, (self.params,))
                    t2 = time.time()
                    logging.info(f"calculate vjp use time: {t2-t1}")
                    calculate_vjp.append(t2-t1)
