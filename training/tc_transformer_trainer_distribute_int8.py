...
class ForwardTextClassificationTrainer:
    ...
    def train_model(self, device=None):
        ...
        origin_state_dict = copy.deepcopy(self.model.state_dict())
        ...
        with torch.no_grad():
            for epoch in range(0, self.args.epochs):
                for batch_idx, batch in enumerate(self.train_dl):
                    ...
                    self.model.load_state_dict(origin_state_dict)
                    quantized_model = torch.quantization.quantize_dynamic(self.model, dtype=torch.qint8)
                    loss = get_loss(quantized_model,x,labels,self.num_labels)
                    
                    # with autocast():
                    #     loss = get_loss(self.model,x,labels,self.num_labels)

                    h = 0.01
                    v_params = tuple([torch.randn_like(p) if p.requires_grad == True else torch.zeros_like(p) for p in self.params])
                    # logging.info(len(v_params))
                    # logging.info(len(self.model.state_dict()))
                    for i,k in enumerate(self.model.state_dict()):
                        self.model.state_dict()[k].add_(h*v_params[i])
                    quantized_model = torch.quantization.quantize_dynamic(self.model, dtype=torch.qint8)
                    terbulence_loss = get_loss(quantized_model,x,labels,self.num_labels)
                    # with autocast():
                    #     terbulence_loss = get_loss(self.model,x,labels,self.num_labels)
