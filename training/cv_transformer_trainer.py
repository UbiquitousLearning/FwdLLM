...
class ForwardTextClassificationTrainer:
    ...
    def eval_model(self, epoch=0, global_step=0, device=None):
        ...
        if isinstance(self.test_dl,torch.utils.data.DataLoader):
            test_sample_len = len(self.test_dl.dataset)
        else:
            test_sample_len = (n_batches-1) * self.args.train_batch_size + len(self.test_dl[-1][0])        
...
class TextClassificationTrainer:
    ...
    def eval_model(self, epoch=0, global_step=0, device=None):
        ...
        # logging.info("out_label_ids = " + str(out_label_ids))
        # logging.info(preds)
        # logging.info(out_label_ids)
        # logging.info(preds.shape)
        # logging.info(out_label_ids.shape)
        ...
        if self.args.model_type == "distilbert":
            ...
            for layer_idx in self.freeze_layers:
                if layer_idx == "e":
                    ...
                    modules.append(model.bert.encoder.layer[int(layer_idx)])
        # elif self.args.model_type == "resnet18" or self.args.model_type == "resnet50":
        #     modules = [model.conv1,model.bn1,model.layer1,model.layer2,model.layer3,model.layer4]
        # for module in modules:
        #     for param in module.parameters():
        #         param.requires_grad = False
