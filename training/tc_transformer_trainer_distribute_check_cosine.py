...
class ForwardTextClassificationTrainer:
    ...
    def train_model(self, device=None):
        ...
        self.true_grad = [torch.zeros_like(p) for p in self.params]
        ...
        for epoch in range(0, self.args.epochs):
            for batch_idx, batch in enumerate(self.train_dl):
                ...
                for i,p in enumerate(self.model.parameters()):
                    if p.grad != None:
                        self.true_grad[i] += copy.deepcopy(p.grad.data)
                ...
                with torch.no_grad():
                    ...
                    for k in range(100000):
                        ...
                        # grad_of_selected_layer.append(jvp*v_params[layer_id])

                        

                        if k+1 in [1,10,100,1000,10000,100000,1000000,10000000,100000000]:
                            index = 0
                            for name,v in self.model.named_parameters():
                                # logging.info(index)
                                if v.requires_grad:
                                    true_grad = self.true_grad[index]
                                    forward_grad = self.grad[index]

                                    # true_grad = torch.flatten(true_grad)
                                    # forward_grad = torch.flatten(forward_grad)

                                    cos_sim = torch.cosine_similarity(true_grad, forward_grad,dim=-1).mean()

                                    logging.info(f"k = {k+1}, param_name = {name}, cos_sim = {cos_sim}, v_shape = {v.shape}")
                                index += 1
                        # if k+1 in [50,100,500,2000,10000]:
                        #     t = torch.stack(grad_of_selected_layer,dim=0)
                        #     v_num = t.shape[0]
                        #     t1, t2 = t[:v_num//2].mean(axis=0), t[v_num//2:].mean(axis=0)
                        #     t = torch.stack([t1,t2])
                        #     var = t.var(dim=0).mean()
                        #     logging.info(f"k = {k}, var = {var}")
                global_step += 1
